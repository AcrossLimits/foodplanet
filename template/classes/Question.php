<?php
    ...
    class Question
    {
        ...
        public function getQuestion($ID) {
            ...
            if (!$result) {
	            ...
	            while($row = mysqli_fetch_array($result) ){
                    ...
                    $question->id = $row[0];
                    $question->description = trim(preg_replace('/\s\s+/', ' ', $row[2]));
                ...
                // Get provider
                $query3 = "SELECT * from tblQuestionProvider WHERE questionID=".$question->id."";
                
                $result3 = mysqli_query($connection, $query3);
                if (!$result3) {
                   $question->provider = "unknown";
	            }
                else {
                    $fullData = array();
	                while($row = mysqli_fetch_array($result3) ){
                        // Get basic question info
                        $question->provider = $row[2];
                    }
        ...
        public function getUnverified() {
            ...
            $questions = array();
            $question = new Question;
            $query = "SELECT A.id as questionID, A.authorID as authorID, B.name as authorName, B.surname as authorSurname, A.statusID as status, A.title as title, A.imgName as image, D.text as answerName, D.imgURL as answerImage, A.description FROM tblQuestion A INNER JOIN tblUser B on A.authorID = B.id INNER JOIN tblQuestionAnswer C on A.id = C.questionID INNER JOIN tblAnswer D on C.answerID = D.id WHERE A.statusID = 2 OR A.statusID = 5;";
            $result = mysqli_query($connection, $query);
